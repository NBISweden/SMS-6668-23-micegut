...
sub check_infernal_version
{
    ...
    if (defined $parts[2])
    {
        my @version_parts = split(/\./, $parts[2]);
        $version = $version_parts[scalar(@version_parts)-1];
    }
    if ($version < 2)
    {
...
sub decode_nci_tRNA_properties
{
    ...
    {
        $trna->seq($mat_seq);
        $log->warning("Mismatch mature sequence for tRNA with noncanonical intron\t". $trna->tRNAscan_id());
    }
